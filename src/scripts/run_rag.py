...
from src.rag.retrievers import RAGManager, RAGRetriever
from src.utils.helpers import extract_model_answer, create_rag_prompt
from src.evaluation.metrics import EvaluationMetrics

def run_rag_experiment(model, dataset, vector_db, retrieval_base_dir, output_dir, top_k=20, max_questions=None):
    """Run the RAG experiment on a dataset."""
    ...
    Based on the following context, answer the following multiple-choice question.
    ...
    for question in tqdm(questions, desc="Processing questions"):
        # Get retrievals for this question
        retrievals = RAGManager.load_retrievals(
            exam=question.exam,
            question_number=question.number,
            top_k=top_k,
            base_dir=retrieval_base_dir,
            retrieval_type='full_question'
        )
        
        # If no retrievals found, create a retriever and get them
        if not retrievals:
            print(f"No retrievals found for question {question.number}, generating them now...")
            retriever = RAGRetriever(vector_db)
            results = retriever.retrieve_for_question(question, top_k=top_k)
            
            # Extract content from results
            retrievals = [r["content"] for r in results]
            
            # Save the retrievals
            retriever.save_retrievals(
                question=question,
                retrievals=results,
                output_dir=os.path.join(retrieval_base_dir, 'full_question'),
                top_k=top_k
            )
        
        # Join retrievals into a context string
        context = ".\n".join(retrievals)
        ...
        results_file = os.path.join(output_dir, f"rag_top_{top_k}_results_{timestamp}.csv")
    ...
    metrics_file = os.path.join(output_dir, f"rag_top_{top_k}_metrics_{timestamp}.json")
    with open(metrics_file, 'w') as f:
        json.dump(combined_metrics, f, indent=2)
    
    # Generate plots
    plot_file = os.path.join(output_dir, f"rag_top_{top_k}_category_accuracy_{timestamp}.png")
...
def main():
    parser = argparse.ArgumentParser(description='Run RAG experiments on neurology questions')
    ...
    parser.add_argument('--output_dir', type=str, default='results/rag', help='Directory to save results')
    parser.add_argument('--retrieval_dir', type=str, required=True, help='Directory containing/for retrievals')
    ...
    parser.add_argument('--top_k', type=int, default=20, help='Number of retrievals to use')
    ...
    # Run experiment
    results_df, metrics = run_rag_experiment(
        model=model,
        dataset=dataset,
        vector_db=vector_db,
        retrieval_base_dir=args.retrieval_dir,
